...
function utilIntToHex(n, pad) {
    ...
    // Handle 64-bit values properly
    if (typeof n === 'bigint') {
        // For BigInt values, convert to string directly
        var s = n.toString(16).toUpperCase();
    } else {
        // For regular numbers, handle negative values
        if (n < 0) {
            // For 64-bit values, use proper two's complement
            if (pad > 8) {
                n += Math.pow(2, 64);
            } else {
                n += Math.pow(2, 32);
...
function Register(name, type, id) {
    ...
    this._update_int = function () {
        try {
            var value;
            if (this.type === 'i64') {
                value = e.reg_read_i64(this.id);
            } else {
                value = e.reg_read_i32(this.id);
            }
            // Debug logging for register values
            if (this.name === 'RAX' || this.name === 'RSP' || this.name === 'RBP' || this.name === 'RIP' || this.name === 'RDI' || this.name === 'RSI') {
                console.log(`Register ${this.name}: ${value} (0x${value.toString(16)})`);
            }
        } catch (error) {
            console.warn('Register read failed:', error);
            return;
        }
...
var paneAssembler = {
    ...
    editPaste: function () {
        var asm = window.prompt("Paste assembly code here (one instruction per line):\n\nExample:\nmov eax, 0\nmov ecx, 5\nloop_start:\ninc eax\ndec ecx\njnz loop_start");
        if (asm) {
            // Use the global assembly function if available
            if (typeof assembleAndLoadFromText !== 'undefined') {
                assembleAndLoadFromText(asm);
            } else {
                // Fallback to old method
                console.log('Using fallback assembly method');
...
$(document).ready(function () {
    // Split.js initialization removed - current layout uses flexbox instead
